...
export async function getReviewsActivity_db({ following }) {
    ...
    const { data, error } = await supabase.from("reviews").select("id, content, rating, published, users(name, slug, image, title), books(name)").in("user_id", followersIds).gte("published", getWeekRange()).limit(3);
